...
//   useEffect(() => {
//     const allRequests = JSON.parse(localStorage.getItem("swapRequests")) || [];
//     const allUsers = JSON.parse(localStorage.getItem("allUsers")) || []; // array of all users with {email, name}
...
//     // Map names for incoming and outgoing requests
//     const incomingRequests = allRequests
//       .filter(r => r.to === userEmail)
//       .map(r => ({ ...r, from: getName(r.from) }));

//     const outgoingRequests = allRequests
//       .filter(r => r.from === userEmail)
//       .map(r => ({ ...r, to: getName(r.to) }));
...
//   useEffect(() => {
//     const allRequests = JSON.parse(localStorage.getItem("swapRequests")) || [];
//     const allUsers = JSON.parse(localStorage.getItem("allUsers")) || []; // {email, name}
...
//   const handleAccept = (id) => {
//     // Update state
//     const updatedIncoming = incoming.filter(r => r.id !== id);
//     setIncoming(updatedIncoming);

//     // Update localStorage
//     const allRequests = JSON.parse(localStorage.getItem("swapRequests")) || [];
//     const remainingRequests = allRequests.filter(r => r.id !== id);
//     localStorage.setItem("swapRequests", JSON.stringify(remainingRequests));
...
export default function RequestsPage() {
  ...
  useEffect(() => {
    ...
    const incomingRequests = allRequests
      .filter(r => r.to === userEmail && r.status === "Pending")
      .map(r => ({ ...r, from: getName(r.from) }));
  ...
  const handleAccept = (id) => {
    ...
    setIncoming(incoming.filter(r => r.id !== id)); // remove from pending incoming
    setOutgoing(updatedRequests.filter(r => r.from === userEmail)); // refresh outgoing
    ...
    <div className="p-6">
      ...
      {outgoing.length === 0 ? <p>No outgoing requests</p> : (
        <table className="w-full border">
          ...
          <tbody>
            {outgoing.map(r => (
              <tr key={r.id} className={`border p-2 ${r.status === "Accepted" ? "bg-green-100" : r.status === "Rejected" ? "bg-red-100" : "bg-yellow-100"}`}>
                {/* <td className="border p-2">{r.to}</td> */}
                <td className="border p-2">{r.offered}</td>
                <td className="border p-2">{r.requested}</td>
                <td className="border p-2">{r.status}</td>
